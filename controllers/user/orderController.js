  ...
  try {
    ...
    if (couponCode) {
      const coupon = await couponModel.findOne({
        ...
        $or: [{ isList: true }, { isReferralCoupon: true }],
      });
    ...
    res
      .status(201)
      .json({ success: true, message: 'Order placed successfully', orderId: newOrder.orderId });
  } catch (error) {
...
const getOrders = async (req, res) => {
  try {
    ...
    const orders = await orderModel.find(query).sort({ createdOn: -1 });
...
const cancelReturnRequest = async (req, res) => {
  try {
    ...
    if (item.status !== 'return_requested') {
      return res
        .status(400)
        .json({ success: false, message: 'No return request found for this item' });
    }
...
const verifyPayment = async (req, res) => {
  try {
    ...
    const selectedAddress = userAddressDoc.address.find(
      (addr) => addr._id.toString() === orderData.addressId
    );
    if (!selectedAddress)
